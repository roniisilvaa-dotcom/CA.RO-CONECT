...
// GET: verificacao de webhook pela Meta
export async function GET(request) {
  ...
  if (mode === 'subscribe' && token === (process.env.META_VERIFY_TOKEN || 'caroconnect2024')) {
  ...
  try {
        for (const entry of body.entry || []) {
                for (const event of entry.messaging || []) {
                          // Ignora echo (mensagens enviadas pela propria conta)
                  if (event.message?.is_echo) continue
                          // Ignora se nao tiver texto
                  ...
                  if (!channels.length) {
                              console.warn('Nenhum tenant com canal Instagram configurado')
                              continue
                  }
                  ...
                  // Verifica se mensagem esta vazia apos remover tags
                  ...
                  await sendInstagramDM(senderId, result.message)
                          console.log(`Resposta Instagram enviada para ${senderId}`)
        ...
        if (err.response?.data) {
                console.error('Detalhe do erro:', JSON.stringify(err.response.data))
        }
  }
