...
fontFamily: 'inherit', outline: 'none', background: '#f9fafb', lineHeight: 1.6,
}
...
color: active ? '#fff' : '#374151', fontSize: 13.5,
...
color: active ? '#fff' : '#374151', fontSize: 11, fontWeight: 700,
...
function startEditFaq(faq, globalIdx) { setEditIdx(globalIdx); setEditQ(faq.q); setEditA(faq.a); setEditTag(faq.tag) }
function saveEditFaq(globalIdx) { setFaqs(f => f.map((x, i) => i === globalIdx ? { ...x, q: editQ, a: editA, tag: editTag } : x)); setEditIdx(null) }
function startEditRule(idx) { setEditRuleIdx(idx); setEditRuleLabel(rules[idx].label) }
function saveEditRule(idx) { setRules(r => r.map((x, i) => i === idx ? { ...x, label: editRuleLabel } : x)); setEditRuleIdx(null) }
function startEditPhrase(idx) { setEditPhraseIdx(idx); setEditPhraseVal(phrases[idx]) }
function saveEditPhrase(idx) { setPhrases(p => p.map((x, i) => i === idx ? editPhraseVal : x)); setEditPhraseIdx(null) }

async function handlePdfUpload(e) {
const files = Array.from(e.target.files || [])
if (!files.length) return
setPdfUploading(true); setPdfStatus(null)
for (const file of files) {
try {
setPdfStatus({ type: 'info', message: 'Lendo "' + file.name + '"...' })
const text = await extractTextFromPdf(file)
if (!text.trim()) { setPdfStatus({ type: 'error', message: '"' + file.name + '" não contém texto legível.' }); continue }
setPdfStatus({ type: 'info', message: 'Enviando "' + file.name + '" para a base de conhecimento...' })
const res = await fetch('/api/train/knowledge', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, filename: file.name }) })
const data = await res.json()
if (!res.ok) throw new Error(data.error || 'Erro ao salvar')
setUploadedDocs(prev => [...prev, { name: file.name, chars: data.chars_added, date: new Date().toLocaleDateString('pt-BR') }])
setPdfStatus({ type: 'success', message: data.message })
} catch (err) { setPdfStatus({ type: 'error', message: 'Erro em "' + file.name + '": ' + err.message }) }
}
...
function getGlobalIdx(faq) { return faqs.findIndex(x => x === faq) }

return (
<div style={{ padding: '24px 28px', maxWidth: 900 }}>
<div style={{ marginBottom: 20 }}>
<h1 style={{ fontSize: 22, fontWeight: 700, color: '#111b21', letterSpacing: '-0.3px' }}>Treinar IA</h1>
<p style={{ fontSize: 13.5, color: '#4b5563', marginTop: 4 }}>Base de conhecimento da sua assistente — carregada com o seu Playbook.</p>
</div>

<div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: '#e9f5ee', borderRadius: 10, border: '1px solid #b7e4c7', marginBottom: 20 }}>
...
{uploadedDocs.length > 0 && ' · ' + uploadedDocs.length + ' PDF' + (uploadedDocs.length > 1 ? 's' : '') + ' adicionado' + (uploadedDocs.length > 1 ? 's' : '')}
</span>
</div>

<div style={{ display: 'flex', gap: 4, background: '#f0f2f5', borderRadius: 24, padding: 4, marginBottom: 24, width: 'fit-content', flexWrap: 'wrap' }}>
...
<Tab label="📄 Documentos PDF" active={tab === 'pdfs'} count={uploadedDocs.length || undefined} onClick={() => setTab('pdfs')} />
</div>
...
{TAGS.map(t => (
<button key={t} onClick={() => setFilter(t)} style={{ padding: '6px 14px', borderRadius: 20, border: '1.5px solid ' + (filter === t ? '#25D366' : '#d1d5db'), background: filter === t ? '#e9f5ee' : '#fff', color: filter === t ? '#0a5c44' : '#374151', fontSize: 13, fontWeight: filter === t ? 600 : 500, cursor: 'pointer', fontFamily: 'inherit' }}>{t}</button>
))}
</div>
...
Perguntas & Respostas
<span style={{ marginLeft: 8, fontSize: 11.5, background: '#e9f5ee', color: '#0a5c44', padding: '2px 8px', borderRadius: 10, fontWeight: 600 }}>{filteredFaqs.filter(f => f.active).length} ativas</span>
</span>
<button onClick={() => setAdding(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
...
return (
<div key={globalIdx} style={{ padding: '14px 20px', borderBottom: fi < filteredFaqs.length - 1 ? '1px solid #f3f4f6' : 'none', background: isEditing ? '#fafffe' : 'transparent', opacity: faq.active ? 1 : 0.5 }}>
{isEditing ? (
<div>
<div style={{ fontSize: 12, fontWeight: 600, color: '#6b7a85', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.4 }}>Editando</div>
<input value={editQ} onChange={e => setEditQ(e.target.value)} placeholder="Pergunta" style={{ ...INPUT_STYLE, marginBottom: 8, fontWeight: 600 }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} autoFocus />
<textarea value={editA} onChange={e => setEditA(e.target.value)} placeholder="Resposta da IA" rows={3} style={{ ...INPUT_STYLE, resize: 'vertical', marginBottom: 10 }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} />
<div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
<span style={{ fontSize: 13, color: '#4b5563', fontWeight: 500 }}>Categoria:</span>
{['Vendas','Curso','Geral','Objeção','Palestra'].map(t => (<button key={t} onClick={() => setEditTag(t)} style={{ padding: '4px 12px', borderRadius: 20, cursor: 'pointer', fontFamily: 'inherit', border: '1.5px solid ' + (editTag === t ? '#25D366' : '#d1d5db'), background: editTag === t ? '#e9f5ee' : 'transparent', color: editTag === t ? '#0a5c44' : '#374151', fontSize: 12.5, fontWeight: editTag === t ? 600 : 500 }}>{t}</button>))}
<div style={{ flex: 1 }} />
<button onClick={() => saveEditFaq(globalIdx)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 16px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 20, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}><IconCheck /> Salvar</button>
<button onClick={() => setEditIdx(null)} style={{ padding: '7px 14px', background: '#f3f4f6', color: '#374151', border: '1px solid #d1d5db', borderRadius: 20, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>Cancelar</button>
</div>
</div>
) : (
...
<span style={{ fontSize: 10.5, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: tc.bg, color: tc.text, flexShrink: 0 }}>{faq.tag}</span>
</div>
<div style={{ fontSize: 13.5, color: '#1a6b5a', lineHeight: 1.6, background: '#f0faf6', borderRadius: 8, padding: '9px 12px', borderLeft: '3px solid #25D366' }}>{faq.a}</div>
</div>
...
<ActionBtn onClick={() => startEditFaq(faq, globalIdx)} title="Editar" color="#128C7E" bg="#e9f5ee" hoverBg="#d1f0e5"><IconEdit /></ActionBtn>
<ActionBtn onClick={() => setFaqs(f => f.filter((_, i) => i !== globalIdx))} title="Excluir" color="#dc2626" bg="#fef2f2" hoverBg="#fee2e2"><IconTrash /></ActionBtn>
</div>
</div>
)}
</div>
...
{adding && (
<div style={{ background: '#fff', borderRadius: 12, border: '2px solid #25D366', padding: '16px 20px', marginBottom: 12 }}>
<div style={{ fontSize: 14, fontWeight: 700, color: '#111b21', marginBottom: 12 }}>Nova pergunta</div>
<input placeholder="Pergunta (ex: Como funciona a consultoria?)" value={newQ} onChange={e => setNewQ(e.target.value)} style={{ ...INPUT_STYLE, marginBottom: 8 }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} autoFocus />
<textarea placeholder="Resposta que a IA vai dar..." value={newA} onChange={e => setNewA(e.target.value)} rows={3} style={{ ...INPUT_STYLE, resize: 'vertical', marginBottom: 10 }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} />
<div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 14 }}>
<span style={{ fontSize: 13, color: '#4b5563', fontWeight: 500 }}>Categoria:</span>
{['Vendas','Curso','Geral','Objeção','Palestra'].map(t => (<button key={t} onClick={() => setNewTag(t)} style={{ padding: '4px 12px', borderRadius: 20, cursor: 'pointer', fontFamily: 'inherit', border: '1.5px solid ' + (newTag === t ? '#25D366' : '#d1d5db'), background: newTag === t ? '#e9f5ee' : 'transparent', color: newTag === t ? '#0a5c44' : '#374151', fontSize: 12.5, fontWeight: newTag === t ? 600 : 500 }}>{t}</button>))}
</div>
<div style={{ display: 'flex', gap: 8 }}>
<button onClick={addFaq} style={{ padding: '9px 20px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 20, fontSize: 13.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Salvar</button>
<button onClick={() => { setAdding(false); setNewQ(''); setNewA('') }} style={{ padding: '9px 16px', background: '#f3f4f6', color: '#374151', border: '1px solid #d1d5db', borderRadius: 20, fontSize: 13.5, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Cancelar</button>
</div>
</div>
)}
</div>
...
<button key={t} onClick={() => setTone(t)} style={{ padding: '8px 16px', borderRadius: 20, border: '1.5px solid ' + (tone === t ? '#25D366' : '#d1d5db'), background: tone === t ? '#e9f5ee' : '#fff', color: tone === t ? '#0a5c44' : '#374151', fontSize: 13.5, fontWeight: tone === t ? 700 : 500, cursor: 'pointer', fontFamily: 'inherit' }}>{t}</button>
))}
</div>
</div>
</div>
<div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
<div style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6' }}><div style={{ fontSize: 15, fontWeight: 700, color: '#111b21' }}>Identidade da assistente</div></div>
<div style={{ padding: '16px 20px', borderBottom: '1px solid #f3f4f6' }}>
<div style={{ fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 8 }}>Nome</div>
<input value={aiName} onChange={e => setAiName(e.target.value)} style={{ ...INPUT_STYLE, width: 280 }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} />
...
<textarea value={bio} onChange={e => setBio(e.target.value)} rows={4} style={{ ...INPUT_STYLE, resize: 'vertical' }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} />
</div>
</div>
<div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
<div style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6' }}>
...
{phrases.map((p, i) => (
<div key={i} style={{ padding: '12px 20px', borderBottom: i < phrases.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
{editPhraseIdx === i ? (
<div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
<span style={{ color: '#25D366', fontSize: 16, paddingTop: 8, flexShrink: 0 }}>🌸</span>
<input value={editPhraseVal} onChange={e => setEditPhraseVal(e.target.value)} style={{ ...INPUT_STYLE, flex: 1, fontStyle: 'italic' }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} autoFocus />
...
<button onClick={save} style={{ padding: '10px 24px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 20, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Salvar personalidade</button>
...
<div style={{ background: '#f0faf6', borderRadius: 10, padding: '10px 14px', fontSize: 13.5, color: '#1a6b5a', lineHeight: 1.6, borderLeft: '3px solid #25D366', fontStyle: 'italic' }}>{p.exemplo}</div>
</div>
</div>
))}
</div>
...
<span style={{ background: section.tagBg, color: section.tagColor, fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 10, letterSpacing: 0.5, textTransform: 'uppercase' }}>{section.label}</span>
...
return (
<div key={ri} style={{ padding: '12px 20px', borderBottom: ri < rules.filter(r => r.type === section.type).length - 1 ? '1px solid #f3f4f6' : 'none', background: isEditingRule ? '#fafffe' : 'transparent' }}>
{isEditingRule ? (
<div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
<span style={{ color: section.iconColor, fontSize: 16, flexShrink: 0 }}>{section.icon}</span>
<input value={editRuleLabel} onChange={e => setEditRuleLabel(e.target.value)} style={{ ...INPUT_STYLE, flex: 1 }} onFocus={e => e.target.style.borderColor = '#25D366'} onBlur={e => e.target.style.borderColor = '#d1d5db'} autoFocus />
...
<div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
<button onClick={save} style={{ padding: '10px 24px', background: '#25D366', color: '#fff', border: 'none', borderRadius: 20, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Salvar regras</button>
{saved && <span style={{ fontSize: 13.5, color: '#0a5c44', fontWeight: 600 }}>✓ Salvo!</span>}
</div>
</div>
)}
...
<div style={{ padding: '14px 18px', background: '#fef9ec', borderRadius: 10, border: '1px solid #fcd34d', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
<span style={{ fontSize: 20, flexShrink: 0 }}>📚</span>
<div>
<div style={{ fontSize: 14, fontWeight: 700, color: '#92400e', marginBottom: 4 }}>Como funciona o treinamento por PDF</div>
<div style={{ fontSize: 13.5, color: '#78350f', lineHeight: 1.6 }}>
Faça upload de PDFs com materiais da Camila — apostilas, guias de estilo, roteiros de palestra — e a IA aprende com esses documentos. O texto extraído é adicionado à base de conhecimento e usado em todas as conversas.
</div>
</div>
</div>
...
<div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
<div style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6' }}>
<div style={{ fontSize: 15, fontWeight: 700, color: '#111b21' }}>Enviar documentos</div>
<div style={{ fontSize: 13, color: '#4b5563', marginTop: 2 }}>Formatos aceitos: PDF com texto (não funciona com PDFs digitalizados/imagens)</div>
</div>
<div style={{ padding: '20px' }}>
<input ref={fileInputRef} type="file" accept=".pdf" multiple onChange={handlePdfUpload} style={{ display: 'none' }} id="pdf-upload-input" />
<label htmlFor="pdf-upload-input" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, padding: '36px 20px', border: '2px dashed #d1d5db', borderRadius: 12, cursor: pdfUploading ? 'not-allowed' : 'pointer', background: pdfUploading ? '#f9fafb' : '#fafffe', transition: 'border-color 0.2s, background 0.2s' }}
onMouseEnter={e => { if (!pdfUploading) { e.currentTarget.style.borderColor = '#25D366'; e.currentTarget.style.background = '#f0faf6' }}}
onMouseLeave={e => { e.currentTarget.style.borderColor = '#d1d5db'; e.currentTarget.style.background = pdfUploading ? '#f9fafb' : '#fafffe' }}>
{pdfUploading ? (
<>
...
<div style={{ fontSize: 14, fontWeight: 600, color: '#0a5c44' }}>Processando...</div>
</>
) : (
<>
<div style={{ width: 52, height: 52, borderRadius: 14, background: '#e9f5ee', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#128C7E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
<div style={{ fontSize: 14, fontWeight: 700, color: '#111b21', marginBottom: 4 }}>Clique para selecionar PDF(s)</div>
<div style={{ fontSize: 13, color: '#6b7a85' }}>Você pode enviar vários arquivos de uma vez</div>
</div>
<div style={{ padding: '8px 20px', background: '#25D366', color: '#fff', borderRadius: 20, fontSize: 13.5, fontWeight: 600 }}>Selecionar arquivos</div>
</>
)}
</label>

{pdfStatus && (
<div style={{ marginTop: 14, padding: '12px 16px', borderRadius: 10, display: 'flex', alignItems: 'flex-start', gap: 10, background: pdfStatus.type === 'success' ? '#e9f5ee' : pdfStatus.type === 'error' ? '#fef2f2' : '#eff6ff', border: '1px solid ' + (pdfStatus.type === 'success' ? '#b7e4c7' : pdfStatus.type === 'error' ? '#fecaca' : '#bfdbfe') }}>
<span style={{ fontSize: 16, flexShrink: 0 }}>{pdfStatus.type === 'success' ? '✅' : pdfStatus.type === 'error' ? '❌' : '⏳'}</span>
<span style={{ fontSize: 13.5, color: pdfStatus.type === 'success' ? '#0a5c44' : pdfStatus.type === 'error' ? '#dc2626' : '#1d4ed8', fontWeight: 500 }}>{pdfStatus.message}</span>
</div>
)}
</div>
</div>
...
<div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
<div style={{ padding: '14px 20px', borderBottom: '1px solid #f3f4f6' }}>
<div style={{ fontSize: 15, fontWeight: 700, color: '#111b21' }}>
Documentos adicionados nesta sessão
<span style={{ marginLeft: 8, fontSize: 11.5, background: '#e9f5ee', color: '#0a5c44', padding: '2px 8px', borderRadius: 10, fontWeight: 600 }}>{uploadedDocs.length}</span>
</div>
</div>
{uploadedDocs.map((doc, i) => (
<div key={i} style={{ padding: '12px 20px', borderBottom: i < uploadedDocs.length - 1 ? '1px solid #f3f4f6' : 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
...
<div style={{ flex: 1 }}>
<div style={{ fontSize: 13.5, fontWeight: 600, color: '#111b21' }}>{doc.name}</div>
<div style={{ fontSize: 12, color: '#6b7a85', marginTop: 2 }}>{(doc.chars / 1000).toFixed(1)}k caracteres extraídos · {doc.date}</div>
</div>
<span style={{ fontSize: 11.5, fontWeight: 700, background: '#e9f5ee', color: '#0a5c44', padding: '3px 10px', borderRadius: 10 }}>✓ Na base</span>
</div>
))}
</div>
)}

<div style={{ padding: '12px 16px', background: '#f0f2f5', borderRadius: 10, fontSize: 13, color: '#4b5563', lineHeight: 1.6 }}>
💡 <strong>Dica:</strong> Envie apostilas do Curso Geração do Estilo, guias de coloração pessoal, roteiros de palestra ou qualquer material escrito pela Camila. Quanto mais a IA aprender, mais ela vai soar como a Camila.
</div>
</div>
)}
</div>
