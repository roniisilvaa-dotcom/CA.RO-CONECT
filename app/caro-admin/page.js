...
const CLIENT_META = {
  'camila-rocha': { name: 'Camila Rocha', role: 'Consultora de Estilo', color: '#C4924A', init: 'CR' },
}
function getMeta(id) {
  if (CLIENT_META[id]) return CLIENT_META[id]
  const parts = (id || 'unk').split('-')
  const colors = ['#4F9CF9','#9B77FF','#FF6B9D','#00D2AA','#FF8C42','#E1306C']
  const idx = id ? id.charCodeAt(0) % colors.length : 0
  return {
    name: parts.map(w => w ? w[0].toUpperCase() + w.slice(1) : '').join(' '),
    role: 'Cliente CA.RO',
    color: colors[idx],
    init: parts.slice(0,2).map(w => w ? w[0].toUpperCase() : '').join(''),
  }
}
    ...
    <div style={{display:'flex',alignItems:'flex-end',gap:3,height:44}}>
      {data.map((d,i) => (
        <div key={i} title={`${d.l}: ${d.v}`}
          style={{flex:1,height:`${Math.max(4,(d.v/max)*100)}%`,background:color||C.gold,borderRadius:'2px 2px 0 0',opacity:0.5+0.5*(d.v/max),minWidth:4,transition:'height .4s ease'}}/>
      ))}
    </div>
  )
}

function Overview({ clients, convos, loading }) {
  ...
  return (
    <div>
              ...
              return (
                <div key={s}>
                  ...
                  <div style={{height:5,background:C.surf3,borderRadius:3}}>
                    <div style={{height:'100%',width:`${pct}%`,background:colors[s],borderRadius:3,transition:'width .5s ease'}}/>
                  </div>
                </div>
              )
            })}
      ...
      <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:'18px 20px'}}>
        <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:14}}>Clientes na Plataforma</div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))',gap:10}}>
          {loading ? (
            <div style={{color:C.muted,fontSize:13}}>Carregando...</div>
          ) : clients.length === 0 ? (
            <div style={{color:C.muted,fontSize:13}}>Nenhum cliente encontrado</div>
          ) : clients.map(cl => {
            ...
            return (
              <div key={cl.id} style={{background:C.surf2,borderRadius:12,padding:'12px 14px',border:`1px solid ${C.borderL}`}}>
                <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:8}}>
                  <div style={{width:34,height:34,borderRadius:'50%',background:`linear-gradient(135deg,${meta.color},${meta.color}77)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:800,color:'#fff',flexShrink:0}}>
                    {meta.init}
                  </div>
                  <div>
                    <div style={{fontSize:13,fontWeight:600,color:C.text,lineHeight:1.2}}>{meta.name}</div>
                    <div style={{fontSize:11,color:C.muted}}>{cl.count} conversas</div>
                  </div>
                </div>
                <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                  {cl.hasWA && <span style={{fontSize:10,padding:'2px 7px',borderRadius:20,background:'#25D36622',color:'#25D366',fontWeight:600}}>WhatsApp</span>}
                  {cl.hasIG && <span style={{fontSize:10,padding:'2px 7px',borderRadius:20,background:'#E1306C22',color:'#E1306C',fontWeight:600}}>Instagram</span>}
                </div>
              </div>
            )
          })}
...
function Clientes({ clients, convos, onSelectClient, onGoTo }) {
    ...
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:22}}>
        <div>
          ...
          <p style={{margin:'5px 0 0',fontSize:13,color:C.muted}}>{clients.length} clientes ativos na plataforma</p>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:14}}>
        {filtered.map(cl => {
          ...
          return (
            <div key={cl.id}
              style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:16,padding:'20px 22px',transition:'border-color .2s,box-shadow .2s',cursor:'default'}}
              ...
              <div style={{display:'flex',alignItems:'flex-start',gap:12,marginBottom:16}}>
                <div style={{width:48,height:48,borderRadius:'50%',background:`linear-gradient(135deg,${meta.color},${meta.color}66)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,fontWeight:900,color:'#fff',flexShrink:0,boxShadow:`0 0 0 3px ${meta.color}22`}}>
                  {meta.init}
                </div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:3,paddingTop:2}}>
                  {cl.hasWA && <Dot color="#25D366" size={8}/>}
                  {cl.hasIG && <Dot color="#E1306C" size={8}/>}
                </div>
              </div>
              ...
              <div style={{display:'flex',gap:6}}>
                <button onClick={() => { onSelectClient(cl.id); onGoTo('conversas') }}
                  style={{flex:1,padding:'9px',borderRadius:9,border:'none',background:meta.color,color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer',transition:'opacity .15s'}}
...
function Conversas({ convos, loading, filterClient, clients }) {
  ...
  const btnSt = (active) => ({
    ...
    background: active ? C.gold : C.surf2, color: active ? '#fff' : C.muted, transition:'all .15s',
  })

  const selSt = {
    padding:'7px 12px', borderRadius:20, border:`1px solid ${C.border}`, background:C.surf2,
    color:C.muted, fontSize:12, outline:'none', cursor:'pointer',
  }
    ...
    <div>
      ...
      <div style={{display:'flex',gap:6,marginBottom:14,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <select value={client} onChange={e => setClient(e.target.value)} style={selSt}>
          ...
          {clients.map(cl => <option key={cl.id} value={cl.id}>{getMeta(cl.id).name}</option>)}
        </select>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:5}}>
        {loading ? (
          ...
          return (
            <div key={c.id}
              ...
              <div style={{width:36,height:36,borderRadius:'50%',background:`linear-gradient(135deg,${meta.color},${meta.color}55)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:800,color:'#fff',flexShrink:0}}>
                {meta.init}
              </div>
              <div style={{flex:1,minWidth:0}}>
        ...
        {filtered.length > 100 && (
          <div style={{padding:14,textAlign:'center',color:C.muted,fontSize:12}}>
            Mostrando 100 de {filtered.length} conversas. Use os filtros para refinar.
          </div>
        )}
      </div>
    </div>
...
function Analytics({ clients, convos, filterClient }) {
  const [selectedClient, setSelectedClient] = useState(filterClient || 'all')
  const maxConvos = Math.max(...clients.map(cl => cl.count), 1)

  const focusClients = selectedClient === 'all' ? clients : clients.filter(cl => cl.id === selectedClient)
    ...
    <div>
      ...
      {selectedClient === 'all' && (
        <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:'20px 22px',marginBottom:14}}>
          <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:16}}>Volume de Conversas por Cliente</div>
              ...
              return (
                <div key={cl.id}>
                  ...
                  <div style={{height:7,background:C.surf3,borderRadius:4,overflow:'hidden'}}>
                    <div style={{height:'100%',width:`${pct}%`,background:`linear-gradient(90deg,${meta.color},${meta.color}88)`,borderRadius:4,transition:'width .6s ease'}}/>
                  </div>
                </div>
              )
            })}
          ...
          return (
            <div key={cl.id} style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:'18px 20px',position:'relative',overflow:'hidden'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
                <div style={{width:40,height:40,borderRadius:'50%',background:`linear-gradient(135deg,${meta.color},${meta.color}66)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:900,color:'#fff',boxShadow:`0 0 0 3px ${meta.color}22`}}>
                  {meta.init}
                </div>
                <div>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:5}}>
                {[
                  ...
                  <div key={s.l} style={{display:'flex',alignItems:'center',gap:8}}>
                    <div style={{fontSize:11,color:C.muted,width:80,flexShrink:0}}>{s.l}</div>
                    <div style={{flex:1,height:4,background:C.surf3,borderRadius:2}}>
                      <div style={{height:'100%',width:cl.count?`${(s.v/cl.count)*100}%`:'0%',background:s.c,borderRadius:2,transition:'width .5s ease'}}/>
                    </div>
...
export default function CaroAdmin() {
  ...
  function goTo(section, clientId) {
    if (clientId !== undefined) setFilterClient(clientId)
    setActive(section)
  }
    ...
    <div style={{ display:'flex', minHeight:'100vh', background:C.bg, fontFamily:'Inter,-apple-system,BlinkMacSystemFont,sans-serif', color:C.text }}>
      ...
      <div style={{width:224,background:C.surf,borderRight:`1px solid ${C.border}`,display:'flex',flexDirection:'column',flexShrink:0,position:'sticky',top:0,height:'100vh',overflowY:'auto'}}>
        <div style={{padding:'22px 18px 18px',borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{fontSize:10,color:C.muted,marginTop:3,letterSpacing:'0.1em',textTransform:'uppercase',fontWeight:600}}>
            Connect Admin
          </div>
        </div>
        ...
        <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,background:C.surf2}}>
          <div style={{display:'flex',alignItems:'center',gap:6}}>
            <div style={{width:7,height:7,borderRadius:'50%',background:loading?C.muted:C.green,flexShrink:0,boxShadow:loading?'none':`0 0 6px ${C.green}`}}/>
            <span style={{fontSize:11,color:loading?C.muted:C.green,fontWeight:600}}>{loading ? 'Carregando...' : 'Sistema online'}</span>
          </div>
        ...
        <div style={{padding:'10px 12px',borderTop:`1px solid ${C.border}`}}>
          <div style={{fontSize:10,color:C.muted,letterSpacing:'0.06em',textTransform:'uppercase',fontWeight:600,marginBottom:8}}>Quick Stats</div>
          {[
            { l:'Clientes', v:clients.length,  c:C.gold   },
            { l:'Conversas',v:convos.length,   c:C.blue   },
        ...
        <div style={{padding:'10px 12px',borderTop:`1px solid ${C.border}`}}>
          <button onClick={load} disabled={loading}
            style={{width:'100%',padding:'9px',borderRadius:9,border:`1px solid ${C.borderL}`,background:'transparent',color:loading?C.muted:C.mutedL,fontSize:12,cursor:loading?'default':'pointer',fontWeight:600,marginBottom:8}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'36px 36px 80px'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          {active === 'overview'  && <Overview  clients={clients} convos={convos} loading={loading}/>}
          {active === 'clientes'  && <Clientes  clients={clients} convos={convos} onSelectClient={id => setFilterClient(id)} onGoTo={s => setActive(s)}/>}
