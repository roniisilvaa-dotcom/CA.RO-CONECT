...
function buildScriptsSection(scripts) {
    ...
    const lines = scripts.map(s => {
          const triggers = s.triggers ? `Gatilhos: ${s.triggers}` : ''
          let response = s.response || ''
          if (s.url) response = response.replace('{URL}', s.url) || s.url
          return `• [${(s.type || 'script').toUpperCase()}] ${s.name}\n  ${triggers ? triggers + '\n  ' : ''}Resposta: ${response}`
    }).join('\n\n')
    return `\n\n══ SCRIPTS & LINKS CONFIGURADOS ══\nQuando o usuário mencionar os gatilhos abaixo, use EXATAMENTE as respostas configuradas:\n\n${lines}`
}

function buildSystemPrompt(config, scripts = []) {
    const scriptsSection = buildScriptsSection(scripts)

  // Se o config já tem system_prompt completo (agente personalizado), usa direto
  if (config.system_prompt) {
        return config.system_prompt + scriptsSection
  }
  ...
  8. Hoje é ${new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}${scriptsSection}
  `.trim()
}
...
export async function processMessage({ tenantId, phone, userMessage, agentConfig, conversationId }) {
  ...
  const config = CUSTOM_AGENTS[tenantId] || agentConfig
    const [history, scripts] = await Promise.all([
          getHistory(tenantId, phone, conversationId),
          getScripts(tenantId),
        ])
  ...
  const response = await client.messages.create({
        ...
        max_tokens: 800,
        system: buildSystemPrompt(config, scripts),
